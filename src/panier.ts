...
function renderCartRow(item: CartItem): string {
  return `
    <tr data-product-id="${item.productId}">
      <td>
        <div class="cart-item-image" style="background-image: url('${item.image}');" role="img" aria-label="${item.name}"></div>
      </td>
      ...
      <td>
        <div class="cart-qty-wrap">
          ...
          <button type="button" aria-label="Augmenter" data-qty="+1">+</button>
        </div>
      </td>
      <td><strong>${formatPrice(item.subtotal)}</strong></td>
      <td>
        <button type="button" class="cart-remove" aria-label="Retirer du panier">Retirer</button>
      </td>
    </tr>
  `;
}
...
export function initPanier(): void {
  initCartBadge();
  renderCart();
